...
if(input.match(GITHUB_LINK_FORMAT)){return[{title:'Shorten link',icon:'linkTemplate.png',action:'shortenLink',actionArgument:input},{title:'Add to things',icon:'linkTemplate.png',action:'addToThings',actionArgument:input}];}// Matching:
// rails/rails#123
...
"use strict";var GraphQL={};GraphQL.execute=function(query,variables){if(!Action.preferences.token){LaunchBar.openURL('http://launchbar-github.com/install');LaunchBar.alert("It looks like this is the first time you're using "+"this action.\n\nWe'll need to authenticate with GitHub in order "+"for everything to work properly. On the following screen, please "+"hit the green Authorize button.");return[];}var requestHeaders={'Authorization':'token '+Action.preferences.token,'User-Agent':'github-launchbar-'+GitHubLB.VERSION};var requestBody={query:query,variables:variables};var result=HTTP.post('https://api.github.com/graphql',{headerFields:requestHeaders,body:JSON.stringify(requestBody)});LaunchBar.debugLog('action=launchbar-github request='+JSON.stringify(requestBody));LaunchBar.debugLog('action=launchbar-github response='+JSON.stringify(result));if(result.data){var body=JSON.parse(result.data);if(body.data){return body;}else{if(body.message){LaunchBar.displayNotification({title:"Couldn't access the GitHub API",string:body.message});}else{LaunchBar.displayNotification({title:"Couldn't access the GitHub API"});}return[];}}};if(typeof module!=='undefined'){module.exports.GraphQL=GraphQL;}
...
'use strict';var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var Resource=function(){function Resource(url){_classCallCheck(this,Resource);this.url=url;}_createClass(Resource,[{key:'toObject',value:function toObject(){var query='\n      query($url: URI!) {\n        resource(url: $url) {\n          __typename\n          ... on Issue {\n            title\n            number\n            repository {\n              name\n              owner {\n                login\n              }\n            }\n          }\n\n          ... on PullRequest {\n            title\n            number\n            repository {\n              name\n              owner {\n                login\n              }\n            }\n          }\n        }\n      }\n    ';var variables={url:this.url};var result=GraphQL.execute(query,variables);if(result){if(result.data&&result.data.resource){var issueOrPullRequest=result.data.resource;var owner=new Account(issueOrPullRequest.repository.owner.login);var repository=new Repository(owner,issueOrPullRequest.repository.name);switch(issueOrPullRequest.__typename){case'Issue':return new Issue(repository,issueOrPullRequest.number,issueOrPullRequest.title);case'PullRequest':return new PullRequest(repository,issueOrPullRequest.number,issueOrPullRequest.title);}}else{return{};}}else{return{};}}}]);return Resource;}();if(typeof module!=='undefined'){module.exports.Resource=Resource;}
