...
// https://developer.obdev.at/launchbar-developer-documentation/#/script-output.
function openAccountRepositories(string){return app.openAccountRepositoriesMenu(string);}function openAccountMenu(string){return app.openAccountMenu(string);}function shortenLink(link,details){return app.shortenLink(link);}function openSettingsMenu(){return app.openSettingsMenu();}function setToken(token){return app.setToken(token);}function openRepositoryMenu(nameWithOwner){var match=nameWithOwner.match(/^(.*)\/(.*)$/);var owner=new Account(match[1]);var repository=new Repository(owner,match[2]);return app.openRepositoryMenu(repository);}
"use strict";var GraphQL={};GraphQL.execute=function(query,variables){if(!Action.preferences.token){LaunchBar.openURL('https://github.com/settings/tokens');LaunchBar.alert("It looks like this is the first time you're using "+"this action.\n\nPlease go to https://github.com/settings/tokens "+"and create token with 'repo' and 'user' scope and set it by invoking "+"the github action and going to settings.");return;}var requestHeaders={authorization:'token '+Action.preferences.token};var requestBody={query:query,variables:variables};var response=[];if(GraphQL._requestIsLocked(requestBody)){return;}else{GraphQL._createRequestLock(requestBody);var result=HTTP.post('https://api.github.com/graphql',{headerFields:requestHeaders,body:JSON.stringify(requestBody)});LaunchBar.debugLog('action=github.lbaction request='+JSON.stringify(requestBody));LaunchBar.debugLog('action=github.lbaction response='+JSON.stringify(result));if(result.data){var body=JSON.parse(result.data);if(body.data){response=body.data;}else{if(body.message){LaunchBar.displayNotification({title:"Couldn't access the GitHub API",string:body.message});}else{LaunchBar.displayNotification({title:"Couldn't access the GitHub API"});}response=[];}}GraphQL._deleteRequestLock(requestBody);return response;}};GraphQL._requestIsLocked=function(body){var requestLocksFile=Action.supportPath+'/request-locks.json';if(!File.exists(requestLocksFile)){LaunchBar.debugLog('action=github.lbaction requestLock=NoLocksFile');File.writeJSON({},requestLocksFile);}if(body in File.readJSON(requestLocksFile)){LaunchBar.debugLog('action=github.lbaction requestLock=locked');return true;}else{LaunchBar.debugLog('action=github.lbaction requestLock=notLocked');return false;}};GraphQL._createRequestLock=function(body){var requestLocksFile=Action.supportPath+'/request-locks.json';var requestLocks=File.readJSON(requestLocksFile);var request=JSON.stringify(body);LaunchBar.debugLog('action=github.lbaction requestLock=creating');requestLocks[request]=true;File.writeJSON(requestLocks,requestLocksFile);};GraphQL._deleteRequestLock=function(body){var requestLocksFile=Action.supportPath+'/request-locks.json';var requestLocks=File.readJSON(requestLocksFile);var request=JSON.stringify(body);LaunchBar.debugLog('action=github.lbaction requestLock=deleting');delete requestLocks[request];File.writeJSON(requestLocks,requestLocksFile);};if(typeof module!=='undefined'){module.exports.GraphQL=GraphQL;}
